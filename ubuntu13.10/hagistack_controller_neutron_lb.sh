...
#neutron db create
sudo mysql -uroot -p$MYSQL_PASS -e "drop database if exists neutron_linux_bridge;"
sudo mysql -uroot -p$MYSQL_PASS -e "create database neutron_linux_bridge character set utf8;"
sudo mysql -uroot -p$MYSQL_PASS -e "grant all privileges on neutron_linux_bridge.* to 'neutron'@'%' identified by '$MYSQL_PASS_NEUTRON';"
sudo mysql -uroot -p$MYSQL_PASS -e "grant all privileges on neutron_linux_bridge.* to 'neutron'@'localhost' identified by '$MYSQL_PASS_NEUTRON';"
sudo mysql -uroot -p$MYSQL_PASS -e "grant all privileges on neutron_linux_bridge.* to 'neutron'@'$NOVA_CONTOLLER_HOSTNAME' identified by '$MYSQL_PASS_NEUTRON';"
...
cat << NEUTRON_OVS | sudo tee /etc/neutron/plugins/linuxbridge/linuxbridge_conf.ini > /dev/null
[vlans]
tenant_network_type=vlan
network_vlan_ranges=physnet1:1000:2999
[database]
connection=mysql://neutron:$MYSQL_PASS_NEUTRON@$NOVA_CONTOLLER_HOSTNAME/neutron_linux_bridge?charset=utf8
retry_interval=2
[linux_bridge]
...
for i in dhcp-agent l3-agent metadata-agent server plugin-linuxbridge-agent neutron-lbaas-agent plugin-vpn-agent
do

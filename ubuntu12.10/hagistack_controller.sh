...
sed -i '8a\log_file = /var/log/keystone/keystone.log' /etc/keystone/keystone.conf
     ...
     15 # compute_port = 7774
     19 # verbose = False
true
     23 # debug = False
true
     58 connection = sqlite:////var/lib/keystone/keystone.db
   connection = mysql://keystone:password@stack01/keystone?charset=utf8
     62 # idle_timeout = 200
     72 # driver = keystone.catalog.backends.templated.TemplatedCatalog
     74 # template_file = default_catalog.templates
     80 # expiration = 86400
...
#glance install
apt-get install -y glance python-jsonschema
...
sed -i "s%sqlite:///glance.sqlite%mysql://glance:$MYSQL_PASS_GLANCE@$NOVA_CONTOLLER_HOSTNAME/glance?charset=utf8%" /etc/glance/glance-api.conf
...
#cinder install
apt-get install cinder-api cinder-common cinder-scheduler cinder-volume -y

#glance setting
cp -a /etc/cinder /etc/cinder_orig

sed -i "s#127.0.0.1#$NOVA_CONTOLLER_HOSTNAME#" /etc/cinder/cinder-api.conf
sed -i "s%sqlite:////var/lib/cinder/cinder.sqlite%mysql://cinder:$MYSQL_PASS_GLANCE@$NOVA_CONTOLLER_HOSTNAME/cinder?charset=utf8%" /etc/cinder/cinder.conf
sed -i "s% sudo /usr/bin/cinder-rootwrap%%" /etc/cinder/cinder.conf
...
mysql -u root -pnova -e "create database cinder;"
mysql -u root -pnova -e "grant all privileges on cinder.* to 'cinder'@'%' identified by '$MYSQL_PASS_GLANCE';"
mysql -u root -pnova -e "grant all privileges on cinder.* to 'cinder'@'localhost' identified by '$MYSQL_PASS_NOVA';"
mysql -u root -pnova -e "grant all privileges on cinder.* to 'cinder'@'$NOVA_CONTOLLER_HOSTNAME' identified by '$MYSQL_PASS_NOVA';"
cinder-manage db sync

#cinder service init
for i in volume api scheduler
do
  start cinder-$i ; restart cinder-$i
done
...
[DEFAULT]
#osapi_compute_listen_port=7774
#metadata_port=7775
#metadata_listen_port=7775
#osapi_volume_listen_port=7776
...
#nova service init
for proc in network compute api objectstore scheduler console consoleauth
do
